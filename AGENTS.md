...
`npx -y @7shep/universal-mcp@alpha install-skills`. By default this writes only to the agent
directories already present in the project (`.agents/skills`, `.claude/skills`, or both — falling
back to `.agents/skills` alone if neither exists yet). Pass `--target=<agents|claude|both>` to
override detection, `--dry-run` to preview without writing, and `--cwd=<path>` to target a
different directory. See [README.md](README.md#agent-skill-commands) for the full behavior.
