...
From a repository checkout, with workspace dependencies installed (`pnpm install`; the import
below resolves `@universal/composition-library` through the workspace symlinks pnpm creates, so
skipping this step fails with `ERR_MODULE_NOT_FOUND`):
...
bare `review_implementation({ files })` call. No network access or built artifacts are required
beyond the workspace install — the script runs the TypeScript source directly via Node's
type-stripping loader. Re-run this script and paste its output back into this document if the
example apps or the design-linter rules change — the numbers below must never be hand-edited.

We independently reproduced this run for this document: three consecutive invocations produced
byte-identical JSON output for both examples, confirming the tool is deterministic. The scores
observed (before: 0/100, 12 findings; after: 84/100, 2 findings) match what was originally
reported when this content was first drafted.
