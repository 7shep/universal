...
export function reviewImplementation(files: readonly SourceFile[], visualEvidence?: VisualEvidence, compositionContext?: ReviewCompositionContext): ReviewResult {
  ...
  if (compositionContext?.expectedSignature && compositionContext.recentSignatures?.length) {
    ...
    if (closest >= .75) findings.push({ severity:'error', rule:'cross-run-structural-repetition', message:`The selected composition is ${Math.round(closest * 100)}% similar to a recent structural signature.`, suggestion:'Regenerate with a different composition seed or exclude the repeated hero and navigation pairing.' });
  }
