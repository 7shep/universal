...
import { validateDesignPlan } from '@universal/design-engine';
...
test('rejects incomplete untrusted design plans', () => {
  ...
  assert.equal(validation.ok, false);
  if (!validation.ok) assert.equal(validation.error.path, 'heroComposition');
});
...
test('penalizes recent structural signatures', () => {
  resetCompositionHistory();
  const first = createDesignPlan({
    prompt: 'An editorial culture organization',
    compositionSeed: 7
  });
  const second = createDesignPlan({
    prompt: 'An editorial culture organization',
    compositionSeed: 7
  });
...
test('warns about purple gradients', () => {
  const review = reviewImplementation([
    { path: 'styles.css', content: '.hero { background: radial-gradient(circle, #8b5cf6, #111); }' }
  ]);
  ...
  const review = reviewImplementation([
    {
      path: 'styles.css',
      content:
        '.a{border-radius:32px}.b{border-radius:32px}.c{border-radius:32px}.grid{display:grid;grid-template-columns:repeat(3,1fr)}'
    }
  ]);
...
test('compares the implementation with the expected composition contract', () => {
  ...
  const review = reviewImplementation(files, undefined, {
    expectedSignature: expected,
    recentSignatures: [expected]
  });
  assert.ok(
  ...
  const review = reviewImplementation([
    {
      path: 'styles.css',
      content:
        '.hero { display:grid; grid-template-columns: 1.2fr .8fr; } .panel { border-radius: 6px; }'
    }
  ]);
...
test('requires desktop and mobile visual evidence before a review can pass', () => {
  const files = [
    { path: 'page.tsx', content: '<main><section className="hero">Direction</section></main>' }
  ];
  const review = reviewImplementation(files, {
    screenshots: [{ viewport: 'desktop', location: 'hero-desktop.png' }],
